	...
	Context("New", func() {
		It("returns an error if unsuccessful", func() {
			...
			bucketConfig := mongo.BucketMigrationConfig{
				DataTypesKeptInLegacy: []string{"basal"},
				DataTypesArchived:     []string{"cbg"},
				DataTypesBucketed:     []string{"cbg", "basal"},
			}
			store, err = mongo.NewStores(nil, nil, nil, bucketConfig, 2015)
		...
		It("returns a new store and no error if successful", func() {
			...
			store, err = mongo.NewStores(config, dbReadConfig, dbReadLogger, bucketConfig, 2015)
			Expect(err).ToNot(HaveOccurred())
